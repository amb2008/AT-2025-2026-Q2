...
public class BlueCloseAuto extends LinearOpMode {
    ...
    private final Pose startPose = new Pose(14, 128, Math.toRadians(300));
    private final Pose scorePose = new Pose(33, 77, Math.toRadians(15));
    private final Pose pickup1Pose = new Pose(22, 80, Math.toRadians(180));
    private final Pose pickup2Pose = new Pose(22, 60, Math.toRadians(90));
    private final Pose pickup3Pose = new Pose(22, 40, Math.toRadians(180));
...
//        }
    ...
    private void intakeMacro(){
        ...
        driveRelativeY(-2);
        driveRelativeX(-initialMove);
        headingCorrect(Math.toRadians(270));
        sleep(500);
        new Thread(()->{
            sleep(1000);
            intakeDone = true;
...
//            ballColor = checkColor();
    ...
    public void driveRelativeY(double inches) {
        // Update odometry to get starting pose
        odo.update();
        double startY = odo.getPosition().getY(DistanceUnit.INCH);   // inches
